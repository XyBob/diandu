...
class Image {
     ...
     * @return void
     */
    static public function water($source, $water, $savename=null, $alpha=80, $positon = 3) {
        //检查文件是否存在
      ...
      /*  //图像位置,默认为右下角右对齐
        ...
        /***********增加水印位置 4月28 23585472@qq.com***********/
        switch ($positon) 
         {
            //1顶部居左 
            case 1: $posX=$posY=0; break;
            //4顶部居右
            case 4: $posX = $sInfo['width']-$wInfo['width']; $posY = 0; break;
            //3居中
            case 3: $posX = ($sInfo['width']-$wInfo['width'])/2; $posY = ($sInfo['height']-$wInfo['height'])/2; break;
            //2底部居左
            case 2: $posX = 0; $posY = $sInfo['height']-$wInfo['height']; break;
            //5底部居右
            case 5: $posX = $sInfo['width']-$wInfo['width']; $posY = $sInfo['height']-$wInfo['height']; break;
            default: $posX=$posY=0;
        }
        self::imagecopymerge_alpha($sImage, $wImage, $posX, $posY, 0, 0, $wInfo['width'], $wInfo['height'], $alpha);
         /***********增加水印位置end 4月28 23585472@qq.com***********/
     ...
     * @param int $red 填充色的红色值，默认为255		//2014-05-12 zhoutao 编辑结束
     * @param int $green 填充色的绿色值，默认为255		//2014-05-12 zhoutao 编辑结束
     * @param int $blue 填充色的蓝色值，默认为255		//2014-05-12 zhoutao 编辑结束
     * @return void
     */
    //注释： 此文件于2014-05-12 被 zhoutao0928@sina.com、zhoutao@360shop.cc 编辑(现在，图片会进行填充，默认为白色)。编辑的地方均有标记，要查看原始的代码，请查看ThinkPHP框架的图片处理类
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $red=255, $green=255, $blue=255) {	//2014-05-12 zhoutao 编辑结束
        // 获取原图信息
        ...
        if ($info !== false) {
			...
			/******** 2014-05-12 zhoutao 编辑开始  *********/
            $thumbImg_width = ($width < $maxWidth)?$maxWidth:$width;		//目标资源的宽度
            $thumbImg_height = ($height < $maxHeight)?$maxHeight:$height;	//目标资源的高度
            
            //介于项目需要，对于大小不符合的图片进行填充。即当图片不进行缩放时，则对图片进行填充处理
            $filling_x = 0;
            $filling_y = 0;
            if($width < $maxWidth || $height < $maxHeight)
            {
            	$filling_x = floor(($maxWidth-$width)/2);
            	$filling_y = floor(($maxHeight-$height)/2);
            	#header("Content-type: image/jpeg");
            	#imagejpeg($thumbImg);exit;
            }
            
            if($maxWidth == C('BIG_IMG_SIZE')){		//如果是生成大图
            	if($scale >= 1){	//如果这时原图的宽和高都不够新图的大小。那么大图此时仅仅是原图的一份拷贝
            		$thumbImg_width = $srcWidth;
            		$thumbImg_height = $srcHeight;
            		$filling_x = 0;
            		$filling_y = 0;
            	}
            }
            /******** 2014-05-12 zhoutao 编辑结束  ***********/
            ...
            if ($type != 'gif' && function_exists('imagecreatetruecolor'))
                $thumbImg = imagecreatetruecolor($thumbImg_width, $thumbImg_height);	//2014-05-12 zhoutao 编辑结束
            else
                $thumbImg = imagecreate($thumbImg_width, $thumbImg_height);				//2014-05-12 zhoutao 编辑结束
            
            /****** 2014-05-12 zhoutao 编辑开始   ******/
            $white = imagecolorallocate($thumbImg,$red,$green,$blue);		//生成填充色
            imagefill($thumbImg,0,0,$white);	//填充	
            /****** 2014-05-12 zhoutao 编辑结束  ******/
            ...
            if (function_exists("ImageCopyResampled"))
                imagecopyresampled($thumbImg, $srcImg, $filling_x, $filling_y, 0, 0, $width, $height, $srcWidth, $srcHeight);
            else
                imagecopyresized($thumbImg, $srcImg, $filling_x, $filling_y, 0, 0, $width, $height, $srcWidth, $srcHeight);
            ...
            imagedestroy($srcImg);
//             echo '新图的大小:'.$thumbImg_width.'*'.$thumbImg_height.'<br/>';
//             echo '填充的宽度:'.$filling_x.' 填充的高度'.$filling_y.'<br/>';
//             echo '拷贝的原图大小：'.$width.'*'.$height.'<hr/>';
            return $thumbname;
        }
        return false;
    }
