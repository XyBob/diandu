...
// $config['smtp_delivery'] 是系统设置是否使用smtp发送邮件

$emailer->from('from@email.com');
$emailer->replyto(replyto@email.com');
 
$emailer->use_template('filename', stripslashes('user_language'));
// 使用设计好的模版，filename是文件名，user_language是用户自定的语言。

$emailer->email_address($email);
// 要发送email地址

$emailer->set_subject('邮件标题');
  
$emailer->assign_vars(array(
	'SITENAME' => $config['sitename'],
	'USERNAME' => preg_replace($unhtml_specialchars_match, $unhtml_specialchars_replace, substr(str_replace("\'", "'", $username), 0, 25)),
	'EMAIL_SIG' => (!empty($config['board_email_sig'])) ? str_replace('<br />', "\n", "-- \n" . $config['system_email_sig']) : '')
);
// 分配一些变量给模版。
...
class emailer
{
	...
	// 重置所有数据 (address, template file, etc etc to default
	function reset()
	{
	...
	function use_template($template_file, $template_lang = '')
	{
		...
		{
			$tpl_file = LANGUAGE . 'lang_' . $template_lang . '/email/' . $template_file . '.tpl';

			if (!@file_exists(@eb_realpath($tpl_file)))
			{
				$tpl_file = LANGUAGE . 'lang_' . $config['default_lang'] . '/email/' . $template_file . '.tpl';
   ...
   // Split the specified file up into a string and return it
	...
	{
		if (is_readable(eb_realpath($sourcefile)))
		{
