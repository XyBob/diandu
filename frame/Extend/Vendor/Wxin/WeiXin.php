	...
	function getAccessToken($url, $rec_user_id = 0)
	{
		...
		if($jsondecode != null)
		{
			if (property_exists($jsondecode, "errcode"))
			{
				...
				{
                    log_file("开始刷新令牌，静默登录","business_qrcode");
			...
			if(property_exists($jsondecode, "openid"))
			{
				...
				$arr = array(
					...
					'headimgurl'		=> $userinfo['headimgurl'] ? urldecode($userinfo['headimgurl']) : '',
					#'privilege'		=> $userinfo['privilege'] ? serialize($userinfo['privilege']) : '',
...
#echo $user_model->getLastSql();
                ...
                log_file("weixin.php setSession user_id = ".$user_id,"session");
			}
		}
	}
