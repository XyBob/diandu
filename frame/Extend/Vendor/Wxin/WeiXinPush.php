...
{
    private $sorry_msg = '很抱歉，未找到相关信息，请输入“世界杯”进入投票。'; private $thank_msg = "感谢您宝贵的建议，我们会提交给相关人员参考，感谢您对育儿树的支持，祝您生活愉快。"; private $world_cup_link = '点击进入：http://world-cup.yurtree.com/';
    private $fromUsername;  //发送过来消息的微信openid
    ...
    public function weixin_run()
    {
        $this->responseMsg();
	$ids = '';
		log_file($this->MsgType, 'subscribe');
        log_file('事件:'.$this->Event, 'subscribe');
        switch ($this->MsgType)
        {
            case 'text':
                //关键字回复
            ...
            case 'link':
             ...
             case 'event':
                if($this->Event == 'subscribe')
                {
                    ...
                    log_file('wx_user_info = ' . json_encode($result), 'subscribe');
                    ...
                    log_file('arr = ' . json_encode($arr), 'subscribe');
                    $where = 'openid = "' . (string)$this->fromUsername . '"';
                    log_file('openid = ' . (string)$this->fromUsername, 'subscribe');
					...
					if (!$user_info)
					{
						...
						$city_info = $area_obj->getIdByName($province, $city);
...
//						'pic'	=> 'http://' . $_SERVER['HTTP_HOST'] . $GLOBALS['config_info']['SUBSCRIBE_PIC'],
                        'pic'	=>$GLOBALS['config_info']['SUBSCRIBE_PIC'],
		...
		{
			break;
			#$str = '欢迎来到' . $GLOBALS['config_info']['SHOP_NAME'];
		}
