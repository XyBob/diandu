 ...
 * 
 * @package xajax
 * @version $Id: xajaxResponse.inc.php,v 1.1 2013/04/18 02:20:26 hyb Exp $
...
//扩展类
class icbuy_xajaxResponse extends xajaxResponse
{  
	function add_addoption($sSelectId, $sOptionText, $sOptionValue)
	{  
	...
	//$aOptions为数组，格式为$aOptions[显示名] = 选项值，即$aOptions = Array(显示名=>选项值)
	function add_addoptions($sSelectId, $aOptions)  
    {
        foreach($aOptions as $sOptionText => $sOptionValue)
        {
            $this->add_addoption($sSelectId, $sOptionText, $sOptionValue);
        }
    }

	function add_cleanalloption($sSelectId)
	{
		$sScript = "document.getElementById('".$sSelectId."').options.length=0;";
		$this->addScript($sScript);
	}

	function add_set_selected($sSelectId, $sIndex)
	{
		$sScript = "document.getElementById('".$sSelectId."').options[".$sIndex."].selected=true;";
		$this->addScript($sScript);
	}
}
