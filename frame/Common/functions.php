...
//输出调试信息，函数调用过程
function show_debug_info($log = true)
{
    $bt = debug_backtrace();
    $str = '';
    foreach ($bt AS $v)
    {
        $str .= $v['file'] . ' on line ' . $v['line'];
        $str .= $log ? "\n" : "<br>";
        $str .= $v['function'];
        $str .= $log ? "\n\n" : "<br><br>";
    }
...
//获取二维码
function get_qr_code($user_id = 0)
{
	//判断分销是否开启
...
#log_file(json_encode($arr), 'cb', true);
#log_file($user_obj->getLastSql(), 'cb', true);
...
function get_headimgurl($openid)
{
	...
	if (!$headimgurl)
	{
		...
		$appid = C('APPID');
		$secret = C('APPSECRET');
		require_once('frame/Extend/Vendor/Wxin/WeiXin.php');
		$access_token = (WxApi::getAccessToken($appid, $secret));
...
function get_kf_media_id()
{
	...
	$access_token = WxApi::getAccessToken($appid, $secret);
#dump($access_token);
	$kf_media_id = $GLOBALS['config_info']['KF_MEDIA_ID'];
	$kf_url = $GLOBALS['config_info']['KF_URL'];
	$kf_expire_time = $GLOBALS['config_info']['KF_EXPIRE_TIME'];
	if (!$kf_media_id || !$kf_url || $kf_expire_time < time())
	{
		$file = $GLOBALS['config_info']['QR_CODE_KF'];
		$result = $wx_obj->uploadMedia($access_token, APP_PATH . $file, 'image', false);
log_file('result = ' . $result, 'kf_push');
		$result = json_decode($result, true);
		//保存ACCESS_TOKEN到数据库
		$config_obj = new ConfigBaseModel();
		$config_obj->setConfig('kf_media_id', $result['media_id']);
		$config_obj->setConfig('kf_url', $result['url']);
		$config_obj->setConfig('kf_expire_time', time() + 86400 * 3 - 600);
	}
