...
class WXPayModel extends PaywayModel
{
	 ...
	 */
	public function __construct($is_app=false)
	{
	...
	public function pay_code($order_id = 0, $voucher_amount = 0.00, $qr_pay_mode = 0, $is_sms=false)
	{
		...
		*/
		...
		if($order_id)
		{
			...
			$body			= $GLOBALS['config_info']['SHOP_NAME'] . '-' . $business_info['business_name'] . '消费';
			$logistics_fee 	= $order_info['express_fee'];
		}
//		else if($is_for_business){
//			$total_fee = $voucher_amount;
//			$out_trade_no 	= date('Ymdhms') . '_shop_' . session('user_id').'_'.$business_user_id;
//			//$attach = $business_user_id;
//			$body			= '付款给商铺';
//		}
		else
		{
...
log_file($jsApiParameters, '33', true);
...
log_file('openid = ' . $openid, 'mobile_pay_code');
        ...
        $prepay_id = $unifiedOrder->getAppPrepayId();
        //=========步骤3：使用jsapi调起支付============
        $jsApi->setPrepayId($prepay_id);
	 ...
	 */
	function pay_response($falg=false)
	{
...
//		if(!$xml){
//			$xml = $_POST;
//			log_file('回调数据获取失败','ms_wxpay_response');
//		}
//		log_file('in pay_response:'.'回调成功'.json_encode($xml),'ms_wxpay_response');
		$notify->saveData($xml);
		...
		log_file('返回的数据 :'.$str, 'pay_response');
		...
		log_file('order_id :'.$order_id, 'pay_response');
		//获取微支付的反馈参数
		$trade_no = (isset($data['transaction_id'])) ? $data['transaction_id'] : '';	//获取订单号
		log_file('trade_no :'.$trade_no, 'pay_response');
		...
		if($order_id)
		{
			...
			log_file('order_info :'.json_encode($order_info), 'pay_response');
			/* 检查支付的金额是否相符*/
			$total_fee = floatval($order_info['pay_amount']) * 100;

            if ($order_info['voucher_code']) {
            
			log_file('voucher_code :'.$order_info['voucher_code'], 'pay_response', true);
			...
			log_file('come in 1', 'pay_response', true);
                    return false;
                }
                if ($ticket_money * 100 + $data['total_fee'] != $order_info['pay_amount'] *100)
                {

			log_file('come in 2', 'pay_response', true);
                    return false;
                }
			...
			if ($trade_status == 'SUCCESS')
			{
				...
				if($falg){
					$order_obj->payOrder($trade_no, 'mobile_wxpay');
				}else{
               ...
               /* $account_obj->addAccount($user_id, 1, intval($data['total_fee']) / 100,
				...
				if($order_info['type'] == 1||$order_info['type'] == 2){//到店消费,外卖
					log_file(json_encode($order_info), 'pay_response', true);
					$business_obj = new BusinessModel();
					...
					$account_obj->addAccount($user_id, AccountModel::ONLINE_VOUCHER, intval($data['total_fee']) / 100, '微信充值', $order_id, $trade_no);
					log_file("order_info :微信充值 ", 'wxpay', true);
					if($order_info['type'] == 1){
						$account_obj->addAccount($user_id, AccountModel::STORE_CONSUMPTION, -intval($data['total_fee']) / 100, '店内消费', $order_id, $trade_no);
					}else{
						$account_obj->addAccount($user_id, AccountModel::TAKE_OUT_PAY, -intval($data['total_fee']) / 100, '外卖消费', $order_id, $trade_no);
					}
					...
					/*
                                       ...
                                       $account_obj->addAccount($business_user_id, AccountModel::PAY_TO_BANK, -$forsiness_amount , '打款到银行卡', $order_id, $trade_no);
                                      //自动打款给商家银行卡
					...
					//log_file($account_obj->getLastSql());
				}else if($order_info['type'] == 3){//商家申请费用
					...
					PushModel::wxPush($user_id, 'open_business', $msg);

					//去商家表里修改状态为待审核
					$business_obj = new BusinessModel();
					$business_obj->editBusiness($order_info['business_id'],array('status'=>4));

					$account_obj->addAccount($user_id, AccountModel::ONLINE_VOUCHER, intval($data['total_fee']) / 100, '微信充值', $order_id, $trade_no);
					$account_obj->addAccount($user_id, AccountModel::BUSINESS_APPLY, intval($data['total_fee']) / 100*-1, '商家申请费用支出', $order_id, $trade_no);
					//commission($order_info['order_id'],3);
...
//				log_file($account_obj->getLastSql());
//				$account_obj->addAccount($user_id, 17, -intval($data['total_fee']) / 100, '店内消费', 0, $trade_no);
//				$account_obj->addAccount($business_user_id, 19, intval($data['total_fee']) / 100, '店内消费收入', 0, $trade_no);
//
//				//log_file($account_obj->getLastSql());
...
log_file('row_dd: ' . json_encode($row_dd));
			...
			if ($trade_status == 'SUCCESS')
			{
				...
				//调用账户模型的addAccount充值等额预存款金额
				$account_obj->addAccount($user_id, 1, intval($data['total_fee']) / 100, '微支付充值', 0, $trade_no);
log_file($account_obj->getLastSql(),'wxpay');
				#$this->log_result('打印SQL: ' . $account_obj->getLastSql());
				return 2;
			}
			else
...
log_file('refundResult = ' . json_encode($refundResult), 'wxpay');
	...
	//$out_trade_no是订单表中的paycode,$refund_fee代表退款金额
    function wx_refund($out_trade_no, $refund_fee)
	{
		...
		log_file('out_refund_no = ' .$out_refund_no , 'wxpay');
		$total_fee = round($refund_fee * 100);
		$refund_fee = round($refund_fee * 100);
		$refund = new Refund_pub();
		$refund->setParameter("transaction_id","$out_trade_no");//商户订单号
		$refund->setParameter("out_refund_no","$out_refund_no");//商户退款单号
		$refund->setParameter("total_fee","$total_fee");//总金额
		$refund->setParameter("refund_fee","$refund_fee");//退款金额
		$refund->setParameter("op_user_id",$this->MCHID);//操作员
	...
	//打款方法
	public function play_money($order_id){
		...
		//外卖让利比例是店铺设置的，和商家提现的不一样，后台统一
        $config_obj = new ConfigBaseModel();
        $business_rate=$config_obj->getConfig('all_business_rate_outside');
        $business_rate=($business_rate)/100;
        log_file('sql：'.$config_obj->getLastSql(),'wxpay');
		$rate_amount = intval($order_info['pay_amount'])*($business_rate);
		log_file('让利：'.$rate_amount,'wxpay');
		//实际打给店铺的钱
		$for_business_amount = (intval($order_info['pay_amount']) - $rate_amount)>0.01?intval($order_info['pay_amount']) - $rate_amount:0.01;
		log_file($for_business_amount."---".$order_info['pay_amount']."---".$business_rate,'wxpay');


        //自动打款给商家银行卡
		$bank_card_obj = new BankCardModel();
		$bank_card_info = $bank_card_obj->getBankCardInfo("user_id=".$business_user_id);
        log_file('sql'.$bank_card_obj->getLastSql(),'wxpay',true);
		log_file("bank_info : " . json_encode($bank_card_info), 'wxpay', true);

		//需要保留2位有效数字
        $for_business_amount=round($for_business_amount,2);
		if($bank_card_info){
			$bank_card_id = $bank_card_info['bank_card_id'];
			...
			if($for_business_amount>$config_obj->getConfig("max_auto_pay_amount")){
				...
				if(!$result){
					...
					$remittance_obj->where($where)->save($arr);
					$account_obj=new AccountModel();
					$account_obj->addBusinessAccount($business_user_id, AccountModel::TAKE_OUT_PAY, $for_business_amount , '外卖消费收入', $order_id, $order_info['pay_code'],'1');
					$account_obj->addBusinessAccount($business_user_id, AccountModel::PAY_TO_BANK, -$for_business_amount , '打款到银行卡', $order_id, $order_info['pay_code']);
				}else{
					log_file("remittance_obj : 汇款失败，error：".$result, 'wxpay', true);
					$arr = array(
							"remark" => $result,
