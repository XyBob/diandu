...
//		$url = $msg['order_id'] ? C('DOMAIN') . '/FrontOrder/order_detail/order_id/' . $msg['order_id'] : $msg['url'];
		...
		log_file(json_encode($wx_templates) , 'addMerchantOrder');
		switch($opt)
		{
			case 'refund':
				#$template_id = 'sWViZCQUbemOp-oDle4fthuCGrziM-qS9EGOVDy3QkE';
				//推微信
				$template_id = $wx_templates['refund'];	//退款成功
				$data = array(
					'touser'		=> $openid,
...
//				$push_obj->push($user_id, $push_arr);
            ...
            case 'defuse_refund':
                #$template_id = 'sWViZCQUbemOp-oDle4fthuCGrziM-qS9EGOVDy3QkE';
                //推微信
                $template_id = $wx_templates['defuse_refund'];	//商家拒绝退款，后台介入
                $data = array(
                    'touser'		=> $openid,
                    'template_id'	=> $template_id,
                    'url'			=> $url,
...
//				$push_obj->push($user_id, $push_arr);
				...
				//发货
            case 'deliver':
                $template_id = $wx_templates['deliver'];	//发货通知
                $data = array(
                    'touser'		=> $openid,
                    'template_id'	=> $template_id,
                    'url'			=> $url,
...
//                $push_obj->push($user_id, $push_arr);
                ...
                break;
			case 'apply_refund':
				$template_id = $wx_templates['apply_refund'];		//订单取消通知
				$data = array(
					'touser'		=> $openid,
					'template_id'	=> $template_id,
					'url'			=> $url,
...
//				$push_obj->push($user_id, $push_arr);
            ...
            case 'literature_review':
                $template_id = $wx_templates['literature_review'];		//资料审核通知
                $data = array(
                    'touser'		=> $openid,
                    'template_id'	=> $template_id,
                    'url'			=> $url,
...
//				$push_obj->push($user_id, $push_arr);
			...
			case 'cancel':
				...
				break;
			case 'pay_in_business':
				$template_id = $wx_templates['pay_in_business'];	//店内支付成功通知
				log_file('template_id = ' . $template_id , 'addMerchantOrder');
				$data = array(
					'touser'		=> $openid,
					'template_id'	=> $template_id,
					'url'			=> $url,
				...
				break;
			case 'open_business':
				$template_id = $wx_templates['open_business'];	//店内支付成功通知
				//log_file('template_id = ' . $template_id , 'addMerchantOrder');
				$data = array(
					'touser'		=> $openid,
					'template_id'	=> $template_id,
				...
				break;
            case 'message_push':
                $template_id = $wx_templates['message_push'];	//店内支付成功通知
                //log_file('template_id = ' . $template_id , 'addMerchantOrder');
                $data = array(
                    'touser'		=> $openid,
                    'template_id'	=> $template_id,
                ...
                break;
			case 'deposit_refuse':
				$template_id = $wx_templates['deposit_refuse'];	//提现拒绝
				//log_file('template_id = ' . $template_id , 'addMerchantOrder');
				$data = array(
					'touser'		=> $openid,
					'template_id'	=> $template_id,
				...
				break;
			case 'deposit_pass':
				$template_id = $wx_templates['deposit_pass'];	//店内支付成功通知
				//log_file('template_id = ' . $template_id , 'addMerchantOrder');
				$data = array(
					'touser'		=> $openid,
					'template_id'	=> $template_id,
