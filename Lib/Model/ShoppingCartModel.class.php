...
class ShoppingCartModel extends Model
{
	 ...
	 * @param int $business_id 商品SKU ID
	 ...
	 */
    public function addShoppingCart($item_id, $item_sku_price_id, $item_info,$business_id='')
    {
		//验证参数有效性
		...
		$real_price = floatval($item_info['mall_price']);
		$real_price = $real_price ? $real_price : 0.00;
		//$total_price = floatval($item_info['mall_price']);
		//$total_price = $total_price ? $total_price : 0.00;
		$user_id = session('user_id');
		$user_id = $user_id ? $user_id : 0;
		if($item_sku_price_id){
			$sku_price_obj=new ItemSkuModel();
			$sku_item_info=$sku_price_obj->getSkuPriceInfo('item_sku_price_id='.$item_sku_price_id);
			//log_file(json_encode($sku_item_info),'shopping_cart');
			//log_file($sku_price_obj->getLastSql(),'shopping_cart');
			$real_price=$sku_item_info['sku_price'];
		}
		//商品库存检测
		$item_obj = new ItemModel();
		if(!$item_sku_price_id){
			$stock_enough = $item_obj->checkStockEnough($item_id, $item_sku_price_id, 1);
			//若库存不足，返回0
			if (!$stock_enough)
			{
				echo 'nostock';
			...
			$stock_enough = $item_obj->checkPackageStockEnough($item_id, 1);
		...
		//查看购物车中是否已存在该商品
		/*$where = $this->where;*/
		$where='merchant_id = '.$business_id.' AND user_id = '.session('user_id');
		...
		log_file(json_encode($shopping_cart_info),'shopping_cart');
		if (!$shopping_cart_info)
		{
			...
			$number = 1 + intval($shopping_cart_info['number']);
	 ...
	 */
    public function deleteShoppingCart($shopping_cart_id)
    {
		//检验$shopping_cart_id有效性
		if (!$this->checkShoppingCartIdValid($shopping_cart_id))
		{
			return false;
		}
		$shopping_cart_info=$this->getShoppingCartInfo('shopping_cart_id = '.$shopping_cart_id);
		if($shopping_cart_info['number']<=1) {//数量小于1,直接删除
			//删除购物车表中的该商品
			return $this->where('shopping_cart_id = ' . $shopping_cart_id)->delete();
		}else{//大于一做减一操作
			return $this->where('shopping_cart_id = ' . $shopping_cart_id)->setDec('number');
		}
	 ...
	 */
    public function clearShoppingCart($where)
    {
    ...
    public function getShopingListCount($business_id,$user_id){
        return $this->where('merchant_id='.$business_id.' AND user_id='.$user_id)->count();
    }
    ...
    {
		//获取购物车内所有商品
		$shopping_cart_list = $this->field($fields)->where($where)->order($order)->select();
	...
	//删除用户-商店购物车的信息
	public function delBusinessShoppingCart($business_id){
	return $this->where('merchant_id = '.$business_id.' and user_id = '.session('user_id'))->delete();
	}
}
