...
class DepositApplyModel extends BaseModel
{
    ...
    public function addDepositApply($arr)
    {
		...
		if($arr['role_type']==2){
			...
			if ($business_info['business_left_money'] < $arr['money'])
			{
				//余额不足
				return -2;
			}
			//这里先加冻结池
			if($arr['state']==1){
				$business_obj->setBusiness($business_info['business_id'], array('freeze_money'=>$business_info['freeze_money']+$arr['money']));
				$left = $account_obj->addBusinessAccount($user_id, AccountModel::DEPOSIT_APPLY, $arr['money'] * -1, '提现申请(商家营业额)','','',$arr['role_type']);
			}else{
				//自动体现失败的，转总后台处理
				$business_obj->setBusiness($business_info['business_id'], array('freeze_money'=>$business_info['freeze_money']+$arr['money'],'business_left_money'=>$business_info['business_left_money']-$arr['money']));
			}
        }else{
		...
		$arr['addtime'] = time();
		log_file('插入数据库的值:'.json_encode($arr),'withdrawals');
		//return $account_obj->getLastSql();
		return $this->add($arr);
    }
    ...
    public function getListData($deposit_apply_list, $export = false)
    {
		foreach ($deposit_apply_list AS $k => $v)
		{
			...
			//获取卡号
			$bank_card_info = M('bank_card')->where("bank_card_id='".$v['bank_card_id']."'")->find();
			$deposit_apply_list[$k]['account'] = $bank_card_info['account'];
			$deposit_apply_list[$k]['realname'] = $bank_card_info['realname'];
		}
		$deposit_apply_list = $this->getQiyePayInfo($deposit_apply_list);
		//var_dump($deposit_apply_list);
...
function getQiyePayInfo($deposit_apply_list)
{
    ...
    public function passDepositApply()
    {
		...
		$msg = '';
		$state = 1;
		if ($business_info['freeze_money'] >= $deposit_apply_info['money'])
		{
			$msg = '恭喜，提现申请已通过';
			$arr = array(
				'state'		=> 1,
			...
			//扣除冻结池中的相应金额除
			$business_obj->editBusiness($business_info['business_id'],array(
				'freeze_money'	=> $business_info['freeze_money'] - $deposit_apply_info['money']
			));
	...
	public function passUserDepositApply()
	{
		$success = false;
		//查看是否有效申请
		$deposit_apply_info = $this->getDepositApplyInfo('deposit_apply_id = ' . $this->deposit_apply_id, 'money, state, user_id');
		if (!$deposit_apply_info || $deposit_apply_info['state'] != 0)
		{
			log_file('come in state no 0','zh');
			return false;
		}
    ...
    public function refuseDepositApply()
    {
		$success = false;
		//查看是否有效申请
		$deposit_apply_info = $this->getDepositApplyInfo('deposit_apply_id = ' . $this->deposit_apply_id, '');
		if (!$deposit_apply_info || $deposit_apply_info['state'] != 0)
		{
			return false;
		}
		if($deposit_apply_info['role_type']!=2) {
			...
			$business_obj = new BusinessModel();
			$business_info = $business_obj->getBusinessInfo('user_id = '.$deposit_apply_info['user_id']);
			log_file('店铺信息:' . json_encode($business_info), 'dianpusheng');
			if ($business_info['freeze_money'] >= $deposit_apply_info['money']) {
				//足够
				$arr = array(
						'state' => 2,
				...
				//调用account模型的addAccount方法
				$account_obj = new AccountModel();
				$left = $account_obj->addBusinessAccount($deposit_apply_info['user_id'], AccountModel::REFUSE_DEPOSIT, $deposit_apply_info['money'], '拒绝提现申请：' . $msg,'','','2');
			}else{
				$msg = '冻结池余额不足，无法提现';
				$arr = array(
						'state' => 2,
				...
				//调用account模型的addAccount方法
				$account_obj = new AccountModel();
				$account_obj->addBusinessAccount($deposit_apply_info['user_id'], AccountModel::REFUSE_DEPOSIT, $business_info['freeze_money'], '拒绝提现申请：' . $msg,'','','2');
			}
			$success = $this->editDepositApply($arr);
			log_file('sql语句1：' . $this->getLastSql(), 'dianpusheng');
		}
		return $success;
    }
...
public static function calRealMoney($money)
{
	...
	public function addRemittance($arr)
	{
		...
		$account_obj = new AccountModel();
		$user_id=$arr['user_id'];
		if($arr['state']==1){//只有等于1的时候对
			$account_obj->addBusinessAccount($user_id, AccountModel::DEPOSIT_APPLY, $arr['money'] * -1, '提现(商家营业额)','','',$arr['role_type']);
		}
