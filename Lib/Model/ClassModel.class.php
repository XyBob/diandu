...
class ClassModel extends Model
{
    ...
    //是否在积分商城中显示该分类
    public function updateClassIntegral($class_id, $i){
        if($i){
                $this->setClass($class_id, array('has_all_integral' => 1));
        }else{
                $item_obj = new ItemModel();
                $result = $item_obj->getItemNum('class_id ='.$class_id .' and is_integral = 1');
                
                if(!$result){
                    $this->setClass($class_id, array('has_all_integral' => 0));
                }
        }
    }
}
