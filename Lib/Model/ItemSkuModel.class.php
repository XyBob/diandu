...
*/
    ...
    public function  getSkuValueBySkuId($item_sku_price_id){
        $item_sku_price_info=$this->getSkuInfo($item_sku_price_id);
        $property_value1_id=$item_sku_price_info['property_value1'];
        $property_value2_id=$item_sku_price_info['property_value2'];
        $property_value_obj=new PropertyValueModel();
        if($property_value1_id){
            $property_value1=$property_value_obj->getPropertyValueField($property_value1_id,'property_value');
            $return_data['property_value1']=$property_value1;
        }
        if($property_value2_id){
            $property_value2=$property_value_obj->getPropertyValueField($property_value2_id,'property_value');
            $return_data['property_value2']=$property_value2;
        }
        return $return_data;

    }
    //获取一个商品的sku值
    public function getSkuValue($item_id,$item_type_id){
        $arr_sku  = get_type_sku($item_type_id);
        return $arr_sku;
    }
    public function getSkuPriceList($where){
