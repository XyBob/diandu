...
class OrderModel extends Model
{
	...
	//订单类型
	const IN_BUSINESS = 1;//店内消费
	const VEGETABLE_MARKET =2;//菜市场外卖
	const OPEN_BUSINESS=3;//申请店铺费用
	const COMMON_BUSINESS=4;//普通外卖
	...
	const INTERVENTION            = 6; //总后台介入,退款受理中
	const REFUNDED                = 7; //已退款
	//注:这里如果总后台拒绝退款,会把状态变为已发货,已发货之后的状态都不能退款

	const REFUND_DELIVEING        = 11; //退货寄送中
/*	const REFUND_DELIVER_CONFIRMD = 8; //退货确认收货
	const REFUNDED                = 9; //已退款
	const INTERVENTION            = 10; //总后台介入


	const STOCKUPED               = 10; //已备货状态
	const CHANGING                = 11; //申请换货中
	const CHANGED                 = 12; //已换货
    const CANCEL_ORDER            = 21; //取消付款订单
    const REFUSE_CANCEL           = 22; //商家拒绝取消
    const INTERVENTION            = 23; //总后台介入*/
	...
	//根据状态返回状态字符串
	public function getStateStr($state){
		$str = '';
		switch($state){
...
//        $this->syncOrderStatus($this->getOrderId());
    ...
    public function setOrder($order_id,$arr){
...
die;
	 ...
	 */
    public function addOrder($order_info, $item_info, $coupon,$business_id='')
    {
		//创建订单，将订单基本信息保存到数据库
		...
		$order_info['user_id'] = session('user_id')?session('user_id'):0;

//		if($order_info['type']==2){//菜市场外卖,1是到店消费,3是商家申请的费用
//			//地址信息
...
//		log_file('add_order_coupon sql = ' . $order_coupon_obj->getLastSql() . ', order_coupon_info = ' . json_encode($order_coupon_info), 'addOrder');
		...
		$shopping_cart_obj->delBusinessShoppingCart($business_id);
...
//		$order_info['cost_amount'] = floatval($item_info['cost_amount']);
    ...
    public function deleteOrderAnyStatus()
    {
    ...
    public function payOrder($pay_code, $pay_tag = '')
    {
		...
		#echo $this->getLastSql();
		#die;
		log_file('last_sql :'.$this->getLastSql(), 'pay_response');
     ...
     * @todo 遍历数组增加order_id
     */
    public function changeList($arr,$argument){
            foreach ($arr as $k => $v){
                $arr[$k]['order_id']=$argument;
            }
            return $arr;
    }
...
log_file('useCoupon: sql = ' . $user_vouchers_obj->getLastSql(), 'useCoupon');
    ...
    /**根据订单条件查找相关用户*/
	public function getOrderListWithUser($fields='', $where = '', $order = '', $limit = ''){
        $order_list=$this->field($fields)->where($where)->order($order)->limit()->select();
        $user_obj=new UserModel();
        foreach ($order_list as $k=>$v){
            $user_info=$user_obj->field('headimgurl,nickname')->where('user_id='.$v['user_id'])->find();
            $order_list[$k]['nickname']=$user_info['nickname'];
            $order_list[$k]['headimgurl']=$user_info['headimgurl'];
	...
	/**
	 * 根据传入的查询条件查找订单表中的订单列表with用户信息and商品信息
	 * @author wzh
	 * @param string $fields
	 * @param string $where
	 * @param string $groupby
	 ...
	 * @todo 根据where子句查询订单表中的订单信息，并以数组形式返回
	 */
	public function getOrderListWithUserInfo($fields = '', $where = '', $order = '', $limit = '', $groupby = '')
	{
		$order_list =  $this->field($fields)->join('tp_users as u on u.user_id=tp_order.user_id')->where($where)->group($groupby)->order($order)->limit()->select();
		//var_dump($this->getLastSql());
		foreach($order_list as $k=>$v){
			$order_item_obj = new OrderItemModel($v["order_id"]);
			$order_item_list = $order_item_obj->getOrderItemList();
			$order_list[$k]['order_item_list'] = $order_item_list;
            $order_list[$k]['addtime']=date('Y-m-d H:i:s',$v['addtime']);
			//商品数量
			$order_list[$k]['order_item_num'] = count($order_item_list);
			$order_list[$k]['status_str'] = $this->convertOrderStatus($v['order_status']);
            if($v['order_status']==3 && $v['is_evaluate']==0){
                $order_list[$k]['status_str']='待评价';
            }
            if(count($order_item_list)=='0'){
                $order_list[$k]['shop_name']='';
            }else{
                $order_list[$k]['shop_name']=$order_item_list[0]['item_name'];
            }
//			$sum_money = 0.0;
			//总价
//			foreach($order_item_list as $k2 => $v2){
//				$sum_money += $v2['real_price'];
//			}
//			$order_list[$k]['sum_money'] = $sum_money;
		}
    ...
    public function getListData($order_list)
	{
		foreach ($order_list AS $k => $v)
		{
			$order_list[$k]['order_status_name'] = $this->getStateStr($v['order_status']);
			if($v['order_status']==3 && $v['is_evaluate']==0){
                $order_list[$k]['order_status_name']='待评价';
            }
			$business_obj=new BusinessModel();
			$business_info=$business_obj->getBusinessInfo('business_id = '.$v['business_id']);
			$order_list[$k]['business_img']=$business_info['sign_pic'];
			$order_list[$k]['business_name']=$business_info['business_name'];
			$order_list[$k]['addtime']=date('Y-m-d H:i:s',$v['addtime']);

			$order_item_obj=new OrderItemModel();
			$order_list[$k]['item_num']=$order_item_obj->getOrderItemNum('order_id = '.$v['order_id']);
			$order_item_list=$order_item_obj->getOrderItemList('','order_id = '.$v['order_id']);

			$order_list[$k]['key_words']=$order_item_list[0]['item_name'];
            # 获取快递公司名称
    ...
    public function convertOrderStatus($order_status)
    {
		switch ($order_status)
		{
            ...
            case self::REFUNDING:
                $order_status = '用户申请退款';
                break;
//            case self::REFUSE_CANCEL:
//                $order_status = '拒绝取消订单';
//                break;
            case self::INTERVENTION:
                $order_status = '总后台介入';
                break;
...
/*
	...
	public function getOrderListData($order_list,$customer_name=''){
		...
		foreach($order_list as $k=>$v){
			if($customer_name){
				$user_info=$user_obj->getUserInfo('nickname','user_id = '.$v['user_id'].' AND nickname LIKE "%'.$customer_name.'%"');
			}else{
				$user_info=$user_obj->getUserInfo('nickname','user_id = '.$v['user_id']);
			}
			if($user_info) {
				$order_list[$k]['customer_name'] = $user_info['nickname'];
				$vouchers_info = $vouchers_obj->getVouchersInfo('vouchers_id = ' . $v['coupon_id'], 'title');
				$order_list[$k]['title'] = $vouchers_info['title'];
				$discount_minus_info=$discount_minus_obj->getDiscountMinusInfo('discount_minus_id = '.$v['discount_mins_id'],'title,amount_limit,num');
                $platform_discounts_info=$discount_minus_obj->getDiscountMinusInfo('discount_minus_id = '.$v['platform_discount_id'],'title,amount_limit,num');
				$order_list[$k]['discount_title'] = $discount_minus_info['title'];
				$order_list[$k]['num']=$discount_minus_info['num'];
				$order_list[$k]['amount_limit']=$discount_minus_info['amount_limit'];
				$order_list[$k]['plat_discount_title'] = $platform_discounts_info['title'];
				$order_list[$k]['plat_num']=$platform_discounts_info['num'];
				$order_list[$k]['plat_amount_limit']=$platform_discounts_info['amount_limit'];
	...
	public function editOrderInfo($order_id,$data)
	{
	...
	//统计商品月销量
	public function getMonthSalesVolume($item_id){
		$beginThismonth=mktime(0,0,0,date('m'),1,date('Y'));
		$endThismonth=mktime(23,59,59,date('m'),date('t'),date('Y'));
		$order_item_obj=new OrderItemModel();
		$data=$this->field('count(*) as num')
					->join("left join {$order_item_obj->getTableName()} on {$this->getTableName()}.order_id={$order_item_obj->getTableName()}.order_id")
					->where("{$this->getTableName()}.order_status=3 and {$this->getTableName()}.addtime >= $beginThismonth and {$this->getTableName()}.addtime < $endThismonth and {$order_item_obj->getTableName()}.item_id = $item_id")
					->find();
		return $data;
	}

	//获取订单满减用户信息
//    public function getOrderUserInfo($order_list,$nickname){
//	    $user_obj=new UserModel();
...
//            $order_list[$k]['nickname']=$user_info['nickname'];
//	        //获取优惠活动信息
//            $discount_info=$discount_obj->getDiscountMinusInfo('discount_minus_id = '.$v['platform_discount_id'],'title,amount_limit');
//            $order_list[$k]['discount_minus_title']=$discount_info['title'];
//
//
//        }
//        return $order_list;
//    }
  //链表获取分页count
    public function getOrderWithUserNum($customer_name='',$start_time,$end_time){
        //$start_time=strtotime($start_time);
        //$end_time=strtotime($end_time);
        $user_obj=new UserModel();
        $table_name_user=$user_obj->getTableName();
        $table_name_order=$this->getTableName();
        $where="$table_name_order.platform_discount_id>0 and (($table_name_order.order_status=3 and type=2) or ($table_name_order.order_status=1 and type=1) ) ";
        if($customer_name!=''){
            $where.=" and $table_name_user.nickname like '%$customer_name%'";
        }
        if($start_time!=''){
            $where.=" and $start_time < $table_name_order.addtime";
        }
        if($end_time!=''){
            $where.=" and $end_time > $table_name_order.addtime";
        }
            $data=$this->join("left join $table_name_user on $table_name_order.user_id=$table_name_user.user_id")
                ->where($where)
                ->count();
            return $data;
    ...
    //链表获取分页数据
    public function getOrderWithUserList($customer_name='',$start_time,$end_time,$limit,$order){
        //$start_time=strtotime($start_time);
        //$end_time=strtotime($end_time);
        $user_obj=new UserModel();
        $table_name_user=$user_obj->getTableName();
        ...
        $user_discount_obj=new DiscountMinusModel();
        foreach ($data as $k=>$v){
            $user_discount_info=$user_discount_obj->getDiscountMinusInfo('discount_minus_id = '.$v['platform_discount_id']);
            //$data.= $user_discount_obj->getLastSql();
            $data[$k]['title']=$user_discount_info['title'];
        }
        return $data;
    }
    //链表获取分页count
    public function getOrderWithUserVoucherNum($business_id,$customer_name='',$start_time,$end_time){
        $start_time=strtotime($start_time);
        $end_time=strtotime($end_time);
        $user_obj=new UserModel();
        $table_name_user=$user_obj->getTableName();
        $table_name_order=$this->getTableName();
        $where="$table_name_order.business_id=$business_id and $table_name_order.coupon_id>0 and (($table_name_order.order_status=3 and type=2) or ($table_name_order.order_status=1 and type=1) ) ";
        if($customer_name!=''){
            $where.=" and $table_name_user.nickname like '%$customer_name%'";
        }
        if($start_time!=''){
    ...
    //链表获取分页数据
    public function getOrderWithUserVoucherList($business_id,$customer_name='',$start_time,$end_time,$limit,$order){
        //$start_time=strtotime($start_time);
        //$end_time=strtotime($end_time);
        $user_obj=new UserModel();
        $table_name_user=$user_obj->getTableName();
        ...
        $user_discount_obj=new VouchersModel();
        foreach ($data as $k=>$v){
            $user_discount_info=$user_discount_obj->getVouchersInfo('vouchers_id = '.$v['coupon_id']);
            //$data.= $user_discount_obj->getLastSql();
            $data[$k]['title']=$user_discount_info['title'];
        }
        return $data;
    }//链表获取分页count
    public function getOrderWithUserDiscountNum($business_id,$customer_name='',$start_time,$end_time){
       // $start_time=strtotime($start_time);
        //$end_time=strtotime($end_time);
        $user_obj=new UserModel();
        $table_name_user=$user_obj->getTableName();
        ...
        $where="$table_name_order.business_id=$business_id and $table_name_order.discount_mins_id>0 and (($table_name_order.order_status=3 and type=2) or ($table_name_order.order_status=1 and type=1) ) ";
        if($customer_name!=''){
            $where.=" and $table_name_user.nickname like '%$customer_name%'";
        }
        if($start_time!=''){
    ...
    //链表获取分页数据
    public function getOrderWithUserDiscountList($business_id,$customer_name='',$start_time,$end_time,$limit,$order){
        //$start_time=strtotime($start_time);
        //$end_time=strtotime($end_time);
        $user_obj=new UserModel();
        $table_name_user=$user_obj->getTableName();
        ...
        $user_discount_obj=new DiscountMinusModel();
        foreach ($data as $k=>$v){
            $user_discount_info=$user_discount_obj->getDiscountMinusInfo('discount_minus_id = '.$v['discount_mins_id']);
            //$data.= $user_discount_obj->getLastSql();
            $data[$k]['title']=$user_discount_info['title'];
        }
        return $data;
