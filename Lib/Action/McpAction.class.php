<?php
//管理员后台
class McpAction extends GlobalAction {
	//待处理数据
	...
	function _initialize() {
		...
		if ($user_id) {
			...
			$Users = M("Users");
			$user_infom = $Users->where('user_id = ' . $user_id)->select();
		...
		/*	$user_info['priv_str_arr'] = explode(',', $user_info['priv_str']);*/
			...
			session('business_id',$business_info['business_id']);
			//session('business_id',10);
		...
		if ($this->login_user['role_type'] != 3) redirect(U('/logout')); //已登录但不是管理员，则强制退出要求重新登录
	...
	/*var_dump( $this->my_menu_file);
		...
		if (!in_array(ACTION_NAME, array('index', 'set_menu', 'uploadHandler', 'delImage', 'common_article', 'upload_desc_pic', 'upload_image')) && $user_id != 1)
		{
            ...
            if (stristr(MODULE_NAME, 'ajax') || in_array(MODULE_NAME, array('McpIntegral')) || IS_AJAX) {

            }else if (!$this->checkpriv('/' . MODULE_NAME . '/' . ACTION_NAME, true))
			{
				redirect(U('/mcp'), 5, '您没有访问此操作的权限，请联系系统管理员！');
			}
		}
		...
		$mod_id = 'mcp' == strtolower(MODULE_NAME) ? 2 : $mod_id;
	...
	{
		if (!$this->checkpriv('/McpConfig/set_password', false))
		{
		...
		redirect(U('/McpConfig/set_password'));
	}
	...
	*/
	protected function get_all_my_priv() {
		include_once(CONF_PATH . 'mcp_priv.php');
		...
		foreach ($admin_menu_file as $k => $v) { //$v = array('id' => 'XX', 'name' => 'XX', 'mod_do_url' => '', 'in_menu' => '', 0=>array(),1=>Array()...);
			...
			foreach ($v as $m => $n) { //有效的（即是数组的值）项$n = array('id' => 'XX', 'name' => 'XX', 'mod_do_url' => '/XX-Mcp_XX', 'in_menu' => '');
				if (!is_array($n)) {
					continue;
				}
	...
	*/
	...
	{
		require('Conf/mcp_ajax_priv.php');
	...
	*/
	...
	public function edit_info() {
		...
		if ($act == 'submit') {
			...
			$this->ajaxReturn(U('/mcp/edit_info'), '帐号信息修改成功！', 1);
		}
	...
	public function edit_pass() {
		...
		if ($act == 'submit') {
			...
			$this->ajaxReturn(U('/mcp/edit_pass'), '帐号密码修改成功！', 1);
		}
