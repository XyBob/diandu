 ...
 *
 */
class McpConfigAction extends McpAction
{
     ...
     * @todo
     */
    public function McpConfigAction()
    {
        parent::_initialize();
    ...
    public function advanced_setting()
    {
        ...
        if ($act == 'save') //提交保存时
        {
            ...
            $this->success('恭喜你，设置成功!', '/McoConfig/advanced_setting');
        }
    ...
    public function set_range(){
        ...
        if ($new_range){
            ...
            $res = $business_obj->where('user_id='.session('user_id'))->save($data);
        ...
        $this->assign('old_range', $business_info['range']);
    ...
    public function base_config()
    {
        ...
        if ($act == 'save') //提交保存时
        {
            ...
            $ConfigBaseModel = new ConfigBaseModel();
            $ConfigBaseModel->setConfigs($data);
            $this->success('恭喜你，参数设置成功了!', '/McoConfig/base_config/mod_id/0');
        }
        //用户登录大图
...
//             //用户注册时
    ...
    public function add_link()
    {
        ...
        if ($act == 'submit') {
            ...
            if ($link->addLink($data)) {
                $this->success('恭喜您，友情链接添加成功！', '/McoConfig/list_link');
            } else {
        ...
        $this->assign('action_src', '/McoConfig/list_link/mode_id/0');
    ...
    public function edit_link()
    {
        ...
        if ($act == 'submit') {
            ...
            if (false !== $link->editLink($id, $data)) {
                $this->success('恭喜您，友情链接修改成功！', '/McoConfig/list_link');
            } else {
        ...
        if (!$id || !ctype_digit($id)) {
            $this->error('非法参数！', '/McoConfig/list_link');
        }
        $linkInfo = $link->getLink($id);
        if (!$linkInfo) {
            $this->error('无效ID！', '/McoConfig/list_link');
        }

        $this->assign('link_info', $linkInfo);

        $this->assign('action_title', '友情链接列表');
        $this->assign('action_src', '/McoConfig/list_link/mod_id/0');
    ...
    public function add_menu()
    {
        ...
        if ($action == 'add') {
            ...
            //表单验证
            if (!$title) {
                $this->error('请填写菜单显示文字~', '__APP__/McoConfig/add_menu');
            }
            if (!$menu_type) {
                $this->error('请选择菜单类型~', '__APP__/McoConfig/add_menu');
            }
            ...
            if ($id = $generalArticle->addMenu($data)) {
                $this->success('恭喜您，菜单项添加成功~', '__APP__/McoConfig/list_menu');
            } else {
                $this->error('对不起，菜单项添加失败~', '__APP__/McoConfig/add_menu');
            }
        }
        ...
        $this->assign('action_src', '/McoConfig/list_menu/mod_id/0');
    ...
    public function edit_menu()
    {
        ...
        if ($action == 'edit') {
            ...
            if ($generalArticle->editMenu($id, $data)) {
                $this->success('恭喜您，菜单项修改成功~', '__APP__/McoConfig/edit_menu/id/' . $id);
            } else {
                $this->error('对不起，菜单项修改失败~', '__APP__/McoConfig/edit_menu/id/' . $id);
            }
        }
    ...
    public function set_service_account()
    {
        ...
        if ($act == 'submit') {
            ...
            if ($totalInsert == $successInsert && $totalUpdate == $successUpdate) {
                $this->success('恭喜您，客服设置成功！', '/McoConfig/set_service_account');
            } else {
                $this->error('对不起，客服设置失败，请稍后重试！');
            }
        }
    ...
    public function alipay_response()
    {
        ...
        if ($return_state == 2) {
            ...
            $this->success('恭喜您，充值成功', '/McoConfig/sms_config/mod_id/0');
        } else {
...
ABC;
        ...
        if ($act == 'add') {
            ...
            if ($wx_reply_obj->delWxKwReply($id)) {
                if ($n_id = $wx_reply_obj->addWxKwReply($arr)) {
                    $this->success('修改成功', '/McoConfig/edit_auto_reply/id/' . $n_id . '/mod_id/0');
                }
                $this->error('修改失败');
            }
    ...
    //开关店铺
    public function business_switch(){
        ...
        if(IS_POST){
            ...
            if($is_open==1){
                ...
                $item_list=$item_obj->getItemList('','business_id='.$business_id);
                if(!$item_list){
                    echo 'no_item';exit;
                }
            }
            ...
            $business_obj=new BusinessModel();
            $re=$business_obj->editBusiness($business_id,$arr);
            echo $re;exit;
        }
       // echo json_encode($business_info);exit;
        $this->assign('business_info',$business_info);
        $this->assign('head_title','开关店');
        $this->display();
    }
