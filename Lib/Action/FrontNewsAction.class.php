...
class FrontNewsAction extends FrontAction{
	...
	{	
		...
		//获取文章列表
        $where = 'isuse = 1 AND (user_type = 3 OR user_type = 0) AND article_tag = '."''";//article_tag=""是将article表中这个字段为空的数据当成是新闻资讯的数据
...
//		exit;
	...
	public function notice_detail()
	{
        ...
        $notice_txt_id = M('notice_txt');
        $contents = $notice_txt_id->field('contents')->where('notice_id='.$id)->find();
		$notice_obj = new NoticeModel();
		$noticeInfo = $notice_obj->getNoticeInfo('notice_id = ' .$id, 'title, addtime, description');
        $noticeInfo = array_merge($noticeInfo,$contents);
//        echo $noticeInfo['contents'];
//        exit;
