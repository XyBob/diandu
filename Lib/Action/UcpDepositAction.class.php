...
class UcpDepositAction extends UcpAction
{
	...
	public function UcpDepositAction()
	{
		parent::_initialize();
	}
	...
	/*	echo json_encode($bank_card_list);
		exit;*/
		if(!$bank_card_list){
			$this->error('您还未绑定银行卡,请先绑定','/UcpFinance/bind_bank',3);
		}
		...
		$this->display(APP_PATH . 'Tpl/UcpDeposit/get_deposit.html');
	}
	...
	function deposit_apply()
	{
		...
		if (is_numeric($amount) && is_numeric($bank_card_id) && $pay_password && $user_id)
		{
			...
			$user_info = $user_obj->getUserInfo('pay_password, left_money,frozen_money');
			if ($user_info['pay_password'] != md5($pay_password))
			{
				$this->error('抱歉，支付密码不正确');
			}
			if($user_info['left_money']<$amount){
                $this->error('抱歉，余额不足');
			}
...
//			echo json_encode($success);exit;
			...
			$left_money-=$amount;
			$frozen_money+=$amount;
			$user_obj->setUserMoney($user_id,array('left_money'=>$left_money,'frozen_money'=>$frozen_money));
	...
	function deposit_list($where, $head_title, $opt)
	{
		...
		$this->display(APP_PATH . 'Tpl/UcpDeposit/get_deposit_list.html');
	}
	...
	function deposit_apply_list($where, $head_title, $opt)
	{
		...
		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		$this->display(APP_PATH . 'Tpl/UcpDeposit/get_deposit_apply_list.html');
	}
    ...
    public function deposit_stat_by_year()
    {
		...
		$where = 'state = 1 AND addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' AND user_id = '.session('user_id');
    ...
    {   $year       = $this->_post('year');
        ...
        $where .= ' AND addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' and user_id = '.session('user_id');
		...
		/*echo json_encode($deposit_stat_list);
		exit;*/
        $new_deposit_stat_list = array();
		for ($i = 1; $i <= $count; $i++)
		{
        ...
        $this->display(APP_PATH . '/Tpl/UcpDeposit/deposit_stat.html');
    }
