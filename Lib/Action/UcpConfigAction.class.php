 ...
 *
 */
class UcpConfigAction extends UcpAction
{
     ...
     * @todo
     */
    public function UcpConfigAction()
    {
        parent::_initialize();
    ...
    public function advanced_setting()
    {
        ...
        if ($act == 'save') //提交保存时
        {
            ...
            $this->success('恭喜你，设置成功!', '/UcpConfig/advanced_setting');
        }
    ...
    public function base_config()
    {
        ...
        if ($act == 'save') //提交保存时
        {
            ...
            $ConfigBaseModel = new ConfigBaseModel();
            $ConfigBaseModel->setConfigs($data);
            $this->success('恭喜你，参数设置成功了!', '/UcpConfig/base_config/mod_id/0');
        }
        //用户登录大图
...
//            }
            ...
            if (!$business_rate || (!is_numeric($business_rate) && !is_float($business_rate))) {
                $this->error('对不起，店铺推荐提成请填写整数或小数!', get_url());
            }
            if (!$personal_rate || (!is_numeric($personal_rate) && !is_float($personal_rate))) {
                $this->error('对不起，个人推荐提成请填写整数或小数!', get_url());
            }
...
//            }
            ...
            if (!$join_personal_rate || (!is_numeric($join_personal_rate) && !is_float($join_personal_rate))) {
                $this->error('对不起，个人推荐提成请填写整数或小数!', get_url());
            }
...
//             //用户注册时
    ...
    public function add_link()
    {
        ...
        if ($act == 'submit') {
            ...
            if ($link->addLink($data)) {
                $this->success('恭喜您，友情链接添加成功！', '/UcpConfig/list_link');
            } else {
        ...
        $this->assign('action_src', '/UcpConfig/list_link/mode_id/0');
    ...
    public function edit_link()
    {
        ...
        if ($act == 'submit') {
            ...
            if (false !== $link->editLink($id, $data)) {
                $this->success('恭喜您，友情链接修改成功！', '/UcpConfig/list_link');
            } else {
        ...
        if (!$id || !ctype_digit($id)) {
            $this->error('非法参数！', '/UcpConfig/list_link');
        }
        $linkInfo = $link->getLink($id);
        if (!$linkInfo) {
            $this->error('无效ID！', '/UcpConfig/list_link');
        }

        $this->assign('link_info', $linkInfo);

        $this->assign('action_title', '友情链接列表');
        $this->assign('action_src', '/UcpConfig/list_link/mod_id/0');
    ...
    public function add_menu()
    {
        ...
        if ($action == 'add') {
            ...
            //表单验证
            if (!$title) {
                $this->error('请填写菜单显示文字~', '__APP__/UcpConfig/add_menu');
            }
            if (!$menu_type) {
                $this->error('请选择菜单类型~', '__APP__/UcpConfig/add_menu');
            }
            ...
            if ($id = $generalArticle->addMenu($data)) {
                $this->success('恭喜您，菜单项添加成功~', '__APP__/UcpConfig/list_menu');
            } else {
                $this->error('对不起，菜单项添加失败~', '__APP__/UcpConfig/add_menu');
            }
        }
        ...
        $this->assign('action_src', '/UcpConfig/list_menu/mod_id/0');
    ...
    public function edit_menu()
    {
        ...
        if ($action == 'edit') {
            ...
            if ($generalArticle->editMenu($id, $data)) {
                $this->success('恭喜您，菜单项修改成功~', '__APP__/UcpConfig/edit_menu/id/' . $id);
            } else {
                $this->error('对不起，菜单项修改失败~', '__APP__/UcpConfig/edit_menu/id/' . $id);
            }
        }
    ...
    public function set_service_account()
    {
        ...
        if ($act == 'submit') {
            ...
            if ($totalInsert == $successInsert && $totalUpdate == $successUpdate) {
                $this->success('恭喜您，客服设置成功！', '/UcpConfig/set_service_account');
            } else {
                $this->error('对不起，客服设置失败，请稍后重试！');
            }
        }
    ...
    public function alipay_response()
    {
        ...
        if ($return_state == 2) {
            ...
            $this->success('恭喜您，充值成功', '/UcpConfig/sms_config/mod_id/0');
        } else {
        ...
        if ($act == 'add') {
            ...
            if ($wx_reply_obj->delWxKwReply($id)) {
                if ($n_id = $wx_reply_obj->addWxKwReply($arr)) {
                    $this->success('修改成功', '/UcpConfig/edit_auto_reply/id/' . $n_id . '/mod_id/0');
                }
                $this->error('修改失败');
            }
