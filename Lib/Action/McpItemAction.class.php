 ...
 * acp后台商品类
 */
class McpItemAction extends McpAction
{
    ...
    public function get_search_condition()
    {
        ...
        if ($category_id) {
            $where .= ' AND class_id = ' . $category_id;
        }
    ...
    public function item_list($where, $head_title, $opt)
    {
       ...
       // echo json_encode($item_list);exit;
        ...
        $class_list=$class_obj->getClassList('business_id = '.session('business_id'));
        ...
        $this->display(APP_PATH . 'Tpl/McpItem/get_item_list.html');
    }
    ...
    {
        $this->item_list('isuse = 1 AND stock > 0 AND business_id = '.session('business_id'), '出售中的' . C('ITEM_NAME') . '列表', 'onsale');
    }
    ...
    {
        $this->item_list('isuse = 0 AND business_id ='.session('business_id'), '仓库中的' . C('ITEM_NAME') . '列表', 'store');
    }
    ...
    {
        $this->item_list('stock <= stock_alarm AND stock > 0 AND business_id ='.session('business_id'), '库存警报的' . C('ITEM_NAME') . '列表', 'alarm');
    }
    ...
    {
        $this->item_list('stock <= 0  AND business_id ='.session('business_id'), '售罄的' . C('ITEM_NAME') . '列表', 'sold_out');
    }
    ...
    public function add_item()
    {
        ...
        // 添加商品
        if ($action == 'add') {
            //echo json_encode($this->_post());exit;
            if ($Item->create()) {
                // 商品分类
                $arr_category   = explode('.', I('category_id'));
                $Item->class_id = I('category_id');
                $Item->base_pic = I('J_Preview');
                #echo "<pre>";
                #print_r($_POST);
                #echo "</pre>";
                #die;
                ...
                // 商品图片
                $data_photo = array();
                $arr_pic    = I('post.J_Preview', array());
                ...
                $Item->business_id = $business_info['business_id'];
              ...
              //  echo $Item->getLastSql();exit;
                if ($item_id) {
                    ...
                    $link = U('/McpItem/get_onsale_item_list');
        ...
        $item_obj=new ItemTxtModel();
        // 商品描述
        $this->assign('contents', $item_obj->getItemTxt($item_id));
       // echo $item_obj->getItemTxt($item_id);exit;
        // 自动生成的商品货号
        ...
        //如果有item_id,获取该商品信息
        $item_info=$Item->where('item_id = '.$item_id)->find();
        $this->assign('pic_data', array(
            'batch' => false,
            'name'  => 'J_Preview',
            'url'   => $item_info['base_pic'],
    ...
    {
        $redirect = U('/McpItem/get_onsale_item_list');
        ...
        if ($act == 'edit') {
           // echo json_encode($this->_post());exit;
            ...
            $this->success('恭喜您，' . C('ITEM_NAME') . '编辑成功！','/McpItem/get_onsale_item_list');
        }
    ...
    public function item_set_list($where, $title, $action_name)
    {
        ...
        $this->display(APP_PATH . 'Tpl/McpItem/item_set_list.html');
    }
    ...
    public function new_item_set()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            $redirect  = $redirect ? url_jiemi($redirect) : '/McpItem/new_item_list/mod_id/2';
            ...
            if ($success) {
                $this->success('恭喜你，设置成功', '/McpItem/new_item_list/mod_id/2');
            } else {
    ...
    public function quick_item_set()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            $item_list = $this->_post('choose'); //哪些商品参加活动
            $redirect  = $this->_get('redirect');
            $redirect  = $redirect ? url_jiemi($redirect) : '/McpItem/quick_item_list/mod_id/2';
            ...
            if ($success) {
                $this->success('恭喜你，设置成功', '/McpItem/quick_item_list/mod_id/2');
            } else {
                $this->error('对不起，设置失败!', get_url());
            }
            exit;
    ...
    public function first_buy_item_set()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            $redirect         = $this->_get('redirect');
            $redirect         = $redirect ? url_jiemi($redirect) : '/McpItem/first_buy_item_list/mod_id/2';
            ...
            if ($success) {
                $this->success('恭喜你，设置成功', '/McpItem/first_buy_item_list/mod_id/2');
            } else {
                $this->error('对不起，设置失败!', get_url());
            }
            exit;
    ...
    public function new_shop_item_set()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            $item_list = $this->_post('choose'); //哪些商品参加活动
            $redirect  = $this->_get('redirect');
            $redirect  = $redirect ? url_jiemi($redirect) : '/McpItem/new_shop_item_list/mod_id/2';
            ...
            if ($success) {
                $this->success('恭喜你，设置成功', '/McpItem/new_shop_item_list/mod_id/2');
            } else {
                $this->error('对不起，设置失败!', get_url());
            }
            exit;
    ...
    public function recommend_item_set()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            $item_list = $this->_post('choose'); //哪些商品参加活动
            $redirect  = $this->_get('redirect');
            $redirect  = $redirect ? url_jiemi($redirect) : '/McpItem/recommend_item_list/mod_id/2';
            ...
            if ($success) {
                $this->success('恭喜你，设置成功', '/McpItem/recommend_item_list/mod_id/2');
            } else {
                $this->error('对不起，设置失败!', get_url());
            }
            exit;
    ...
    public function item_sales_num_stat()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            if (count($items) > 1) {
                $this->error('对不起,最多选择一项商品', U('/McpItem/item_sales_num_stat'));
            }
            if (count($items) == 0) {
                $this->error('对不起,最选择需要统计的商品', U('/McpItem/item_sales_num_stat'));
            }
    ...
    public function item_export()
    {
        ...
        if ($act == 'add') //执行添加操作时
        {
            ...
            if (count($items) == 0) {
                $this->error('对不起,最少选择一项商品', U('/McpItem/item_sales_num_stat'));
            }
    ...
    function store_type_list(){
        ...
        $this->assign('business_classify_name',$business_classify_name);
    ...
    function add_to_my_store(){
        ...
        $tpl_item_id=$data['tpl_item_id'];
        $stock=$data['stock'];
        $mall_price=$data['mall_price'];

        //取出模板商品内容
        $item_obj=new ItemModel();
        $item_info=$item_obj->getItemInfo('item_id = '.$tpl_item_id);
        //整理数组,某些字段重新赋值
        $item_info['stock']=$stock;
        $item_info['mall_price']=$mall_price;
        $item_info['business_id']=$business_id;
        $qiniu_data=json_decode($this->qiniu_copy($item_info['base_pic']),true);//qiniu 图片复制
        $item_info['base_pic']=$qiniu_data['img_url'];
        unset($item_info['item_id']);
        // 自动生成的商品货号
        $last_id = intval($item_obj->order('item_id DESC')->getField('item_id'));
        $item_sn = $this->system_config['SN_PREFIX'] . sprintf('%0' . $this->system_config['SN_LENGTH'] . 's', $last_id + 1);
        $item_info['item_sn']=$item_sn;

        $result=$item_obj->addItem($item_info);
        if($result){
            echo 'success';
        }else{
            echo $item_obj->getLastSql();
