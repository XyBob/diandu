...
* @Date 2014-09-04
 */
class UcpOrderAction extends UcpAction
{
    /**
     * 初始化
     * @author 姜伟
	...
	private function get_search_condition()
	{
		...
		if($user_info['area_id']){//说明是区代理,内容为区id
			$where.=' AND area_id = '.$user_info['area_id'];
		}else if($user_info['city_id']){//说明是市代理
			$where.=' AND city_id = '.$user_info['city_id'];
		}else if($user_info['province_id']){//说明是省代理
			$where.=' AND province_id = '.$user_info['province_id'];
		}
	...
	function order_list($where, $head_title, $opt)
	{
		...
		$this->display(APP_PATH . 'Tpl/UcpOrder/get_order_list.html');
	}
	...
	{
		$this->order_list('isuse = 1 and order_status=1', '所有订单', 'ALL');
	}
	...
	public function order_detail()
	{
		...
		$redirect = $redirect ? url_jiemi($redirect) : U('/UcpOrder/get_pre_pay_order_list');
		...
		try
		{
			$order_info = $order_obj->getOrderInfo($order_obj->list_fields . ', weight, system_discount_amount, user_address_id, user_remark, express_number, express_company_id, user_id, province_id, city_id, area_id');
		}
		catch (Exception $e)
		{	
...
#		$this->assign('gift_list', $gift_list);
	...
	private function refund_change_order_list($where, $head_title, $opt)
	{
		...
		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		$this->display(APP_PATH . 'Tpl/UcpOrder/get_refund_change_order_list.html');
	}
	...
	public function order_refund_change_apply_detail()
	{
		...
		$order_id = intval($order_id);
		$redirect = $this->_get('redirect');
		$redirect = $redirect ? url_jiemi($redirect) : U('/UcpOrder/get_pre_refund_order_list');
		if (!$order_id)
		{
        ...
        if ($type) {
            if ($type == 'small_area') {
                if (!$area_id && !$city_id && !$province_id) 
                    $this->error('请选择省市区', U('/UcpOrder/order_stat_by_area'));
                ...
                if ($big_area_id<0) 
                    $this->error('请选择大区', U('/UcpOrder/order_stat_by_area'));
    ...
    function group_order_list($where, $head_title, $opt)
	{
		...
		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		$this->display(APP_PATH . 'Tpl/UcpOrder/get_group_order_list.html');
	}
