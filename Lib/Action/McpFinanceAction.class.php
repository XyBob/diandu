...
class McpFinanceAction extends McpAction {
    ...
    public function get_account_log() {
        ...
        $earns_obj  = D('account');
        $earns      = $earns_obj->where('change_type='.AccountModel::STORE_CONSUMPTION_INCOME.' AND user_id='.$user_id)->sum('amount_in');//earns是总收益
        if($start_time)
        {
        ...
        //按用户id筛选
          ...
          //  $where['change_type'] = AccountModel::STORE_CONSUMPTION_INCOME;
        //获取订单列表
        ...
        $changed_list = $AccountModel->getAccountList('', $where, 'addtime desc');
      // echo json_encode($changed_list);exit;
        //echo $AccountModel->getLastSql();exit;
  ...
  // dump($account_apply_list); 
    ...
    public function recharge_stat_by_day() 
    {
		...
		$recharge_stat_list = $account_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%H") AS hour, COUNT(*) AS recharge_num, SUM(amount_in) AS total_amount')->where('user_id = '.session('user_id').' and addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' and change_type = '.AccountModel::STORE_CONSUMPTION_INCOME )->group('hour')->order('addtime DESC')->select();
    ...
    public function recharge_stat_by_month() 
    {
		...
		//获取充值统计数据
		$account_obj = new AccountModel();
		$recharge_stat_list = $account_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%m") AS day, COUNT(*) AS recharge_num, SUM(amount_in) AS total_amount')->where('user_id = '.session('user_id').' AND addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' and change_type = '.AccountModel::STORE_CONSUMPTION_INCOME )->group('day')->order('addtime DESC')->select();
    ...
    public function recharge_stat_by_year() 
    {
		...
		//获取充值统计数据
		$account_obj = new AccountModel();
		$recharge_stat_list = $account_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%m") AS month, COUNT(*) AS recharge_num, SUM(amount_in) AS total_amount')->where('user_id = '.session('user_id').' AND addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' and change_type = '.AccountModel::STORE_CONSUMPTION_INCOME )->group('month')->order('addtime DESC')->select();
