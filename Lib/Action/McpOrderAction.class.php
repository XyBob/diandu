 ...
 * @Date 2014-09-04
 */
class McpOrderAction extends McpAction
{
    /**
     * 初始化
     * @author 姜伟
    ...
    private function get_search_condition()
    {
        ...
        //初始化SQL查询的where子句
        $where = ' and  type = 1 and business_id = '.$business_id;
...
//        exit;
        ...
        foreach ($order_list as $k=>$v){
            ...
            $payway_info = $payway_obj->getPaywayInfo('pay_tag = '."'".$v[payway]."'", 'pay_name');
            $order_list[$k]['payway']=$payway_info['pay_name'];
            $order_list[$k]['status']=$this->translate_order_status($v['order_status']);
        }
        //$order_list = $order_obj->getListData($order_list);
       /* echo json_encode($order_obj->getLastSql());
        exit;*/

        $this->assign('order_list', $order_list);

        $this->assign('copy_link', strtolower($opt));
        $this->assign('opt', $opt);
        $this->assign('head_title', $head_title);
        $this->display(APP_PATH . 'Tpl/McpOrder/get_order_list.html');
    }
    ...
    public function get_refunding_order_list()
    {
        $this->order_list('isuse = 1 AND (order_status = ' . OrderModel::REFUNDING.' or order_status = '. OrderModel::INTERVENTION.')', '申请退款中的订单', 'CANCELED');
    }

    public function get_refunded_order_list()
    {
        $this->order_list('isuse = 1 AND order_status = ' . OrderModel::REFUNDED, '已退款的订单', 'CANCELED');
    }
    /**
     * 获取所有订单列表
     * @author 姜伟
    ...
    public function order_detail()
    {
        ...
        $redirect = $redirect ? url_jiemi($redirect) : U('/McpOrder/get_pre_pay_order_list');
...
//        exit;
        if($order_info['order_status']==1&&$order_info['type']==1){
            $order_info['status']='已付款';
        }else{
            $order_info['status']=OrderModel::getStateStr($order_info['order_status']);
        }


        $user_obj=new UserModel();
        $user_info=$user_obj->getUserInfo('','user_id = '.$order_info['user_id']);
        $order_info['nickname']=$user_info['nickname'];
        $this->assign('order_info', $order_info);
        //echo json_encode($order_info);exit;
        $business_obj=new BusinessModel();
        $business_info=$business_obj->getBusinessInfo('business_id = '.$order_info['business_id'],'box_fee,start_delivery_fee');
        //调用订单商品模型获取订单商品列表
       ...
       // echo json_encode($user_address_info);exit;
        $address=str_replace(';',' ',$user_address_info['address']);
        $realname=$user_address_info['realname'];
        ...
        foreach ($item_list as $k=>$v){
            $box_count+=$v['number'];
            $item_list[$k]['sum']=$v['mall_price']*$v['number'];//单个商品不同数量的总价格
            $item_info=$item_obj->getItemInfo('item_id = '.$v['item_id']);
            $item_list[$k]['base_pic']=$item_info['base_pic'];
            if($v['item_sku_price_id']!=0){//获取sku属性
                ...
                $item_list[$k]['property_value_one']=$property_value_one['property_value'];
                $item_list[$k]['property_value_two']=$property_value_two['property_value'];
            }
            //$sum_price+=$v['mall_price']*$v['number'];
        }
        $box_fee=$box_count*$business_info['box_fee'];//餐盒费
...
#		$this->assign('gift_list', $gift_list);
    ...
    public function order_export()
    {
        ...
        if ($act == 'export')
        {
            ...
            $objPHPExcel->getProperties()->setCreator("yrtech")
                ->setLastModifiedBy("yrtech")
            ...
            $objPHPExcel->getActiveSheet(0)->setCellValue('Q1', '商品信息');
            $objPHPExcel->getActiveSheet(0)->setCellValue('R1', '管理员留言');
            ...
            foreach ($arr as  $value) {
                ...
                $objPHPExcel->getActiveSheet(0)->setCellValue('O' . $i, $pay_time);
                ...
                $order_item_list = $order_obj->getOrderItemList();
                $item_str = '';
                ...
                $objPHPExcel->getActiveSheet(0)->setCellValue('Q' . $i, $item_str);
    ...
    private function refund_change_order_list($where, $head_title, $opt)
    {
        ...
        $this->assign('opt', $opt);
        $this->assign('head_title', $head_title);
        $this->display(APP_PATH . 'Tpl/McpOrder/get_refund_change_order_list.html');
    }
   ...
   /* public function get_refunded_order_list()
    ...
    public function order_refund_change_apply_detail()
    {
        ...
        $order_id = intval($order_id);
        $redirect = $this->_get('redirect');
        $redirect = $redirect ? url_jiemi($redirect) : U('/McpOrder/get_pre_refund_order_list');
        if (!$order_id)
        {
    ...
    public function order_stat_by_day()
    {
        ...
        //获取订单统计数据
        $order_obj = new OrderModel();
        $order_stat_list = $order_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%H") AS hour, SUM(pay_amount) AS total_amount')->where('addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' AND business_id = '.$business_id .' and type =1')->group('hour')->order('addtime DESC')->select();
    ...
    public function order_stat_by_month()
    {
        ...
        //获取订单统计数据
        $order_obj = new OrderModel();
        $order_stat_list = $order_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%d") AS day, SUM(pay_amount) AS total_amount, SUM(pay_amount - express_fee) AS profit')->where('addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' and business_id = '.$business_id.' and type =1')->group('day')->order('addtime DESC')->select();
    ...
    public function order_stat_by_year()
    {
        ...
        //获取订单统计数据
        $order_obj = new OrderModel();
        $order_stat_list = $order_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%m") AS month, SUM(pay_amount) AS total_amount, SUM(pay_amount -  express_fee) AS profit')->where('addtime >= ' . $start_time . ' AND addtime <= ' . $end_time.' and business_id = '.$business_id.' and type =1')->group('month')->order('addtime DESC')->select();
        ...
        if ($type) {
            if ($type == 'small_area') {
                if (!$area_id && !$city_id && !$province_id)
                    $this->error('请选择省市区', U('/McpOrder/order_stat_by_area'));
                ...
                if ($big_area_id<0)
                    $this->error('请选择大区', U('/McpOrder/order_stat_by_area'));
    ...
    public function deliver_order()
    {
        ...
        $express_company_id = I('post.express_company_id');
    ...
    function group_order_list($where, $head_title, $opt)
    {
        ...
        $this->assign('opt', $opt);
        $this->assign('head_title', $head_title);
        $this->display(APP_PATH . 'Tpl/McpOrder/get_group_order_list.html');
    }
    ...
    function get_new_order()
    {
        ...
        if ($user_id && $user_info && $user_info['role_type'] == 1)
        {
            $order_obj = new OrderModel();
            $order_num = $order_obj->getOrderNum('addtime >= ' . (time() - 2) . ' AND (order_status = ' . OrderModel::PAYED . ')');
            $order_num = $order_num ? $order_num : 0;
            echo $order_num;
            log_file($order_num, 'get_new_order', true);
            #echo 2;
            exit;
        }
    }
    ...
    function delete_item(){
        ...
        $order_obj=new OrderModel($order_id);
        $result=$order_obj->deleteOrderAnyStatus();
        if($result){
            echo 'success';
        }else{
            echo 'fail';
    ...
    function translate_order_status($status){
        if($status==OrderModel::PRE_PAY){
            return '未付款';
        }
        if($status==OrderModel::PAYED){
            return '已付款/等待发货';
        }
        if($status==OrderModel::DELIVERED){
            return '已发货';
        }
        if($status==OrderModel::CONFIRMED){
            return '确认收货';
        }
        if($status==OrderModel::CANCELED){
            return '已取消';
        }
        if($status==OrderModel::REFUNDING||$status==OrderModel::INTERVENTION){
            return '退款中';
        }
        if($status==OrderModel::REFUNDED){
            return '已退款';
        }
    }
}
