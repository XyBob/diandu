...
class McpDepositAction extends McpAction
{
	...
	function deposit_list($where, $head_title, $opt)
	{
		...
		$this->display(APP_PATH . 'Tpl/McpDeposit/get_deposit_list.html');
	}
	...
	function get_deposit(){
		...
		$business_info=$business_obj->getBusinessInfo('business_id = '.session('business_id'));
		$bank_bank_obj = new BankCardModel();
		$bank_obj = new BankModel();
		$where = "user_id='".session("user_id")."'";
		$bank_card_list = $bank_bank_obj->where($where)->select();
		$bank_list = $bank_obj->select();

		foreach($bank_card_list as $key=>$val){
			foreach($bank_list as $key2=>$val2){
				//print_r($val2);
				if($val['bank_id']==$val2['bank_id']){
					$bank_card_list[$key]['bank_name'] = $val2['bank_name'];
				}
			}
		}
		$order_obj=new OrderModel();
		$sum1=$order_obj->query('select sum(pay_amount) as total from tp_order where business_id=61 AND type=1 AND order_status=1'.' AND pay_time >'.strtotime(date('Y-m-d')).' AND pay_time <'.strtotime(date('Y-m-d',strtotime('+1 day'))));
		$deposit_obj=new DepositApplyModel();
		$sum2=$deposit_obj->query('select sum(money) as total from tp_deposit_apply where is_today = 1 and state=0  '.' AND addtime >'.strtotime(date('Y-m-d')).' AND addtime <'.strtotime(date('Y-m-d',strtotime('+1 day'))).' and user_id ='.session('user_id'));
		$this->assign('sum',$sum1[0]['total']-$sum2[0]['total']);
		$this->assign('left_money',$business_info['business_left_money']);
		$this->assign('bank_card_list', $bank_card_list);
		$this->display(APP_PATH . 'Tpl/McpDeposit/get_deposit.html');
	}

	//申请提现
	function deposit_apply()
	{
		$left_money = $this->_post('left_money');
		$sum = $this->_post('sum');
		$amount = $this->_post('amount');
		$bank_card_id = $this->_post('bank_card_id');
		$pay_password = $this->_post('pay_password');
		$is_today = $this->_post('is_today');
		...
		// dump(I('post.'));exit;
		if (is_numeric($amount) && is_numeric($bank_card_id) && $pay_password && $user_id)
		{
			//判断支付密码是否正确
			$user_obj = new UserModel($user_id);
			$user_info = $user_obj->getUserInfo('pay_password, left_money');
			if ($user_info['pay_password'] != md5($pay_password))
			{
				$this->error('抱歉，支付密码不正确');
			}
			...
			$success = $deposit_apply_obj->addDepositApply($arr);

			log_file($user_id,'tixian');
			$business_obj = new BusinessModel();
	...
	function deposit_apply_list($where, $head_title, $opt)
	{
		...
		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		$this->display(APP_PATH . 'Tpl/McpDeposit/get_deposit_apply_list.html');
	}
    ...
    {   $year       = $this->_post('year');
        ...
        $this->display(APP_PATH . '/Tpl/McpDeposit/deposit_stat.html');
    }
