 ...
 * acp后台商品类
 */
class McpDiscountMinusAction extends McpAction {
    ...
    public function get_discount_minus_list()
    {
        ...
        $where  = "merchant_id = $merchant_id" . $this->get_search_condition();
       ...
       /* echo json_encode($discount_list);
        exit;*/
    ...
    public function get_discount_minus_detail()
    {
        ...
        if(empty($discount_id)){
            $where='discount_mins_id>0';
        }else{
            $where='discount_mins_id = '.$discount_id;
        }
...
//        if($customer_name!=''){
            $count=$order_obj->getOrderWithUserDiscountNum($business_id,$customer_name,$start_time,$end_time);
            //echo $count;
//        }
        //echo $count;
        //分页处理
        ...
        $this->assign('show', $show);
        $order='addtime DESC';
        $order_list = $order_obj->getOrderWithUserDiscountList($business_id,$customer_name,$start_time,$end_time,$limit,$order);
...
//        }
    ...
    public function add_discount_minus()
    {
        ...
        if($submit == 'submit')             //执行添加操作
        {
            ...
            if ($success)
            {
                $this->success('恭喜您，满减活动添加成功','/McpDiscountMinus/get_discount_minus_list');
            }
            else
            {
    ...
    public function edit_discount_minus()
    {
        ...
        $discount_minus_id = intval($this->_get('discount_id'));
        $discount_minus_obj = new DiscountMinusModel;
...
//        echo json_encode($discount_minus_info);
//        exit;
        if (!$discount_minus_info)
        {
            $this->error('抱歉，满减活动不存在', U('/McpDiscountMinus/get_discount_minus_list'));
        }
        ...
        if($submit == 'submit')             //执行添加操作
        {
            ...
            if ($success)
            {
                $this->success('恭喜您，满减活动修改成功','/McpDiscountMinus/get_discount_minus_list');
            }
            else
            {
                $this->error('抱歉，修改失败');
...
//        exit;
        ...
        $this->display(APP_PATH . '/Tpl/McpDiscountMinus/add_discount_minus.html');
    }

    //删除满减活动
    function del_discount(){
        $discount_minus_id=$this->_post('discount_minus_id');
        $discount_minus_obj = new DiscountMinusModel();
        $result=$discount_minus_obj->delDiscountMinus($discount_minus_id);
        if($result){
            echo 'success';
        }else{
            echo 'fail';
    ...
    function off_shelf(){
        $discount_minus_id=$this->_post('discount_minus_id');
        $discount_minus_obj = new DiscountMinusModel($discount_minus_id);
        $discount_info=$discount_minus_obj->getDiscountMinusInfo('discount_minus_id = '.$discount_minus_id,'isuse');
        $result=$discount_minus_obj->editDiscountMinus(array('isuse'=>!$discount_info['isuse']));
       // $result=$discount_minus_obj->editDiscountMinus(array('isuse'=>"ABS(isuse-1)"));
        if($result){
            echo 'success';
        }else{
              echo 'fail';
