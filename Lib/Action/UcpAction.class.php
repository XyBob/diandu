<?php
//管理员后台
class UcpAction extends GlobalAction {
	//待处理数据
	...
	function _initialize() {
		...
		if ($this->login_user['role_type'] != 5) redirect(U('/logout')); //已登录但不是管理员，则强制退出要求重新登录
		...
		if (!in_array(ACTION_NAME, array('index', 'set_menu', 'uploadHandler', 'delImage', 'common_article', 'upload_desc_pic', 'upload_image')) && $user_id != 1)
		{
			...
			if (stristr(MODULE_NAME, 'ajax') || in_array(MODULE_NAME, array('UcpIntegral')) || IS_AJAX) {

			}else if (!$this->checkpriv('/' . MODULE_NAME . '/' . ACTION_NAME, true))
			{
				redirect(U('/ucp'), 5, '您没有访问此操作的权限，请联系系统管理员！');
			}
		}
		...
		$mod_id = 'ucp' == strtolower(MODULE_NAME) ? 2 : $mod_id;
	...
	public function index()
	{
		...
		if (!$this->checkpriv('/UcpConfig/set_password', false))
		{
		...
		redirect(U('/UcpConfig/set_password'));
	}
	...
	*/
	protected function get_all_my_priv() {
		include_once(CONF_PATH . 'ucp_priv.php');
	...
	*/
	...
	{
		require('Conf/ucp_ajax_priv.php');
	...
	*/
	...
	public function edit_info() {
		...
		if ($act == 'submit') {
			...
			$this->ajaxReturn(U('/ucp/edit_info'), '帐号信息修改成功！', 1);
		}
	...
	public function edit_pass() {
		...
		if ($act == 'submit') {
			...
			$this->ajaxReturn(U('/ucp/edit_pass'), '帐号密码修改成功！', 1);
		}
