...
class AcpConfigAction extends AcpAction
{
    ...
    public function list_log(){
        ...
        $log_list=$login_obj->getLoginLogList('',"user_id=$user_id","login_time desc");
    ...
    public function set_range(){
        $new_range=$this->_post('new_range');
        $pre_near_num=$this->_post('pre_near_num');

        if ($new_range){
            $data['range']=$new_range;
            $data['pre_near_num']=$pre_near_num;
            $ConfigBaseModel = new ConfigBaseModel();
            $ConfigBaseModel->setConfigs($data);
            $this->success("修改成功");
        }
        $config = $this->system_config;
        $old_range=$config['RANGE'];
        $this->assign('old_range', $old_range);
        $this->assign('config', $this->system_config);
        $this->assign('head_title','搜索范围设置');
        $this->display();
    ...
    public function base_config()
    {
        ...
        if ($act == 'save') //提交保存时
        {
            ...
            $bank_pay_ip                = $this->_post('bank_pay_ip');
            ...
            $qr_code_bg                 = $this->_post('qr_code_bg'); //用户登录大图
            $qr_code_kf                 = $this->_post('qr_code_kf');
            $map_ad                     = $this->_post('map_ad');//地图页面广告图
            $max_auto_pay_amount        = $this->_post('max_auto_pay_amount');//自动打款到银行卡限额
            ...
            $data                = array(
                ...
                'qr_code'                    => $qr_code,
                'qr_code_bg'                 => $qr_code_bg,
                'qr_code_kf'                 => $qr_code_kf,
                'sms_uid'                    => $sms_uid,
                'sms_key'                    => $sms_key,
                'bank_pay_ip'                => $bank_pay_ip,
                'map_ad'                     => $map_ad,
                'max_auto_pay_amount'        => $max_auto_pay_amount,
                 'over_time'                 => $over_time
...
//            $fenxiao_level           = $this->_post('fenxiao_level');
            ...
            $business_rate  = $this->_post('business_rate');
            $personal_rate  = $this->_post('personal_rate');
            $system_rate  = $this->_post('system_rate');
            $all_business_rate  = $this->_post('all_business_rate');
...
//            }
            ...
            if (!$first_level_agent_rate || (!is_numeric($first_level_agent_rate) && !is_float($first_level_agent_rate))) {
                $this->error('对不起，省代理提成请填写整数或小数!', get_url());
            }

            if (!$second_level_agent_rate || (!is_numeric($second_level_agent_rate) && !is_float($second_level_agent_rate))) {
                $this->error('对不起，市代理提成请填写整数或小数!', get_url());
            }

            if (!$third_level_agent_rate || (!is_numeric($third_level_agent_rate) && !is_float($third_level_agent_rate))) {
                $this->error('对不起，区代理提成请填写整数或小数!', get_url());
            }
            if (!$business_rate || (!is_numeric($business_rate) && !is_float($business_rate))) {
                $this->error('对不起，店铺推荐人提成比例请填写整数或小数!', get_url());
            }
            if (!$personal_rate || (!is_numeric($personal_rate) && !is_float($personal_rate))) {
                $this->error('对不起，个人推荐人提成比例请填写整数或小数!', get_url());
            }
            if (!$system_rate || (!is_numeric($system_rate) && !is_float($system_rate))) {
                $this->error('对不起，系统服务费率请填写整数或小数!', get_url());
            }
            if (!$all_business_rate || (!is_numeric($all_business_rate) && !is_float($all_business_rate))) {
                $this->error('对不起，系统店铺消费费率请填写整数或小数!', get_url());
            }
...
//                'fenxiao_level'           => $fenxiao_level,
                ...
                'all_business_rate'  => $all_business_rate,
            );
    ...
    public function outside_fenxiao_config(){
        ...
        if ($act == 'save') //提交保存时
        {
            $is_fenxiao_open         = 1;
//            $fenxiao_level           = $this->_post('fenxiao_level');
            $first_level_agent_rate  = $this->_post('first_level_agent_rate_outside');
            $second_level_agent_rate = $this->_post('second_level_agent_rate_outside');
            $third_level_agent_rate  = $this->_post('third_level_agent_rate_outside');
            $business_rate  = $this->_post('business_rate_outside');
            $personal_rate  = $this->_post('personal_rate_outside');
            $system_rate  = $this->_post('system_rate_outside');
            $all_business_rate  = $this->_post('all_business_rate_outside');
...
//            }
            ...
            if (!$all_business_rate || (!is_numeric($all_business_rate) && !is_float($all_business_rate))) {
                $this->error('对不起，系统外卖消费费率请填写整数或小数!', get_url());
            }

            $data = array(
                'is_fenxiao_open_outside'         => $is_fenxiao_open,
//                'fenxiao_level'           => $fenxiao_level,
                'first_level_agent_rate_outside'  => $first_level_agent_rate,//省级代理提成比例
                'second_level_agent_rate_outside' => $second_level_agent_rate,//市级代理提成比例
                'third_level_agent_rate_outside'  => $third_level_agent_rate,//县、区级代理提成比例
                'business_rate_outside'  => $business_rate,//店铺推荐人提成比例
                'personal_rate_outside'  => $personal_rate,//个人推荐人提成比例
                'system_rate_outside'  => $system_rate,//系统服务费率
                'all_business_rate_outside'  => $all_business_rate,//店铺让利比例
            );
        ...
        $config = $this->system_config;
        $this->assign('config', $this->system_config);
        $this->assign('head_title','外卖分销设置');
        $this->display();
    }

    /**
     * @access public
     * @todo 开店提成比例设置 tp_config表中需要设置的参数
     * @return void
     * @author wzh
     */
    public function business_fenxiao_config()
    {
        $act = $this->_post('act');
        if ($act == 'save') //提交保存时
        {
            $is_fenxiao_open         = 1;
//            $fenxiao_level           = $this->_post('fenxiao_level');
            $join_prov_rate  = $this->_post('join_prov_rate');
            $join_city_rate = $this->_post('join_city_rate');
            $join_area_rate  = $this->_post('join_area_rate');
            $join_business_rate  = $this->_post('join_business_rate');
            $join_personal_rate  = $this->_post('join_personal_rate');
            $system_rate  = $this->_post('system_rate');
            $business_apply  = $this->_post('business_apply');
//            if (!$fenxiao_level || !is_numeric($fenxiao_level) || ($fenxiao_level < 1 || $fenxiao_level > 3)) {
//                $this->error('对不起，分销级数请填写1-3的整数!', get_url());
//            }

            if (!$join_prov_rate || (!is_numeric($join_prov_rate) && !is_float($join_prov_rate))) {
                $this->error('对不起，省代理提成请填写整数或小数!', get_url());
            }

            if (!$join_city_rate || (!is_numeric($join_city_rate) && !is_float($join_city_rate))) {
                $this->error('对不起，市代理提成请填写整数或小数!', get_url());
            }

            if (!$join_area_rate || (!is_numeric($join_area_rate) && !is_float($join_area_rate))) {
                $this->error('对不起，区代理提成请填写整数或小数!', get_url());
            }
            if (!$join_business_rate || (!is_numeric($join_business_rate) && !is_float($join_business_rate))) {
                $this->error('对不起，店铺推荐人提成比例请填写整数或小数!', get_url());
            }
            if (!$system_rate || (!is_numeric($system_rate) && !is_float($system_rate))) {
                $this->error('对不起，系统服务费率请填写整数或小数!', get_url());
            ...
            $data = array(
                'is_fenxiao_open'         => $is_fenxiao_open,
//                'fenxiao_level'           => $fenxiao_level,
                'join_prov_rate'  => $join_prov_rate,
                'join_city_rate' => $join_city_rate,
                'join_area_rate'  => $join_area_rate,
                'join_business_rate'  => $join_business_rate,
                'join_personal_rate'  => $join_personal_rate,
                'system_rate'  => $system_rate,
