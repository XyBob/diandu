 ...
 * acp后台商品类型类
 */
class McpItemTypeAction extends McpAction {
    ...
    function _initialize() {
        ...
        $this->assign('action_src', U('/McpItemType/list_type'));
	...
	public function list_type()
	{
        ...
        $arr_type = $this->Type->where('business_id = '.session('business_id'))->limit($Page->firstRow, $Page->listRows)->order('item_type_id')->select();
    ...
    public function add_type()
    {
        ...
        if ($action == 'add') {
            if ($this->Type->create()) {
                $this->Type->isuse = 1;
                $this->Type->business_id = session('business_id');
                ...
                if ($item_type_id) {
                   ...
                   $this->success('恭喜您，商品类型添加成功！', U('/McpItemType/list_type'));
                } else {
