...
class AcpOrderAction extends AcpAction
{
	...
	private function get_search_condition()
	{
		...
		if($business_name){
			...
			$business_list=$business_obj->getBusinessList('business_name like "%'.$business_name.'%"');
			if($business_list){
				foreach($business_list as $k=>$v){
					$sql_arr[]=$v['business_id'];
				}
				$sql_str=implode(',',$sql_arr);
				$where.=' AND business_id in ('.$sql_str.')';
			}else{
				$where.=' And 0';
			}
		}
...
//		/$order_list = $order_obj->getListData($order_list);
        ...
        /*    $payway_info = $payway_obj->getPaywayInfo('pay_tag = '."'".$v[payway]."'", 'pay_name');
			...
			if($v['payway']=='mobile_alipay'){
				$order_list[$k]['payway']='支付宝app支付';
			}
			if($v['payway']=='alipay'){
				$order_list[$k]['payway']='支付宝支付';
			}

			if($v['payway']=='wxpay'){
				$order_list[$k]['payway']='微信支付';
			}

			if($v['payway']=='mobile_wxpay'){
				$order_list[$k]['payway']='微信app支付';
			}
        }
		//获取商家名字
		$business_obj=new BusinessModel();
		foreach($order_list as $k=>$v){
			...
			$order_list[$k]['business_name']=$business_obj->getBusinessField($v['business_id'],'business_name');
		}
	...
	{
		$this->order_list('isuse = 1 and order_status!=4 and type!=3', '所有订单', 'ALL');
	}

	public function refund_order()
	{
		$this->order_list('isuse = 1 and order_status=6 and type!=3', '总后台介入的退款订单', 'ALL');
	}
...
//        $redirect = $redirect ? url_jiemi($redirect) : U('/AcpUser/get_all_user_list');
        ...
        if (!$user_id)
        {
            $this->error('无效的用户号', U('/AcpOrder/get_all_order_list/'));
        }

        //调用用户模型的getUserInfo获取用户信息
        $user_obj = new UserModel($user_id);
        $user_info = $user_obj->getUserInfo('username, nickname, realname, email, mobile, address, province_id, city_id, area_id, big_area_id, consumed_money, left_money, left_integral, user_rank_id, reg_time, store_sn, user_type, qq, is_enable,role_type');
	...
	function agree_refund(){
		...
		if($flag){//同意
			if($order_info['payway']=="wxpay"||$order_info['payway']=="mobile_wxpay"){
				...
				if($wx_refund_obj->wx_refund($order_info['pay_code'],/*$order_info['pay_amount']*/0.01)){
					...
					$arr['order_status']=7;
					$result=$order_obj->setOrder($order_id,$arr);
				}
			}
			...
			$result=$order_obj->editOrderInfo($order_id,array('order_status'=>OrderModel::DELIVERED));
		}
		if($result){
            //微信状态推送
            if($flag){
                ...
                if($order_info['type'] == 2){
					...
					$push_obj->jpush_user('退款拒绝通知', $order_info['user_id'], 1, $ext_msg);
                }
            }
