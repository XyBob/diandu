...
class AcpMessageAction extends AcpAction {
	...
	public function add_message(){
		...
		if(IS_POST){
			$data['addtime']=time();
			$data['message_type']=1;
			$result=$MessageBaseModel->addMessageByArr($data);
			if($result){
                $msg = array(
                    "first" => "尊敬的用户，您有新的消息",
                    "keyword1" => $data['message_title'],
                    "keyword2" => date('Y-m-d H:i:s'),
                    "remark" => "感谢您的使用",
                    "url"=>"dd.diandupt.com/FrontUserCenter/message_detail/message_id/".$result,
                );
                $user_obj=new UserModel();
                $count=$user_obj->getUserNum();
                $user_obj->setLimit($count);
                $user_list=$user_obj->getUserList('user_id');
				...
				$this->success('添加成功','/AcpMessage/list_message');
			}else{
	...
	public function edit_message(){
		...
		if(IS_POST){
			...
			if($result){
				$this->success('修改成功','/AcpMessage/list_message');
			}else{
