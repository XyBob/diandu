...
//		console.log({'title':item_name,'category':category_id,'brand':brand_id,'cost_price':cost_price});
//		$.jPops.showLoading();
		$.post('/AcpPromo/get_items_ajax',{'title':item_name,'category':category_id,'brand':brand_id,'cost_price':cost_price},function(data){
//			$.jPops.hideLoading();
...
//	console.log(discount);
	...
	if(item_length && type && discount){
		...
		$('.cost').each(function(){
			var cost_price = $(this).html();	//市场价格
			if(cost_price > 0.00)			//市场价大于0的可以参加促销活动
			{
				var new_price = (type == 1)?cost_price * discount:cost_price-discount;
