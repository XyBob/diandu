	...
	$(".j-skupanelCtrl").live("change", function(){
		...
		if(val==1){
            ...
            if (sku_num == 0) {
                ...
                $.jPops.alert({
                    title:"提示",
                    content:"请到<a href='/McpItemType/edit_type/id/" + type_id +"' target='_blank'>“" + item_name + "类型”</a>中添加规格属性。",
                    okBtnTxt:"确定",
                    callback:function(){
                ...
                if (r) {
                    $.validator.setDefaults({
                        submitHandler: function() {
                            ...
                            $.post("/McpItemAjax/add_extend_prop", {'p_type': p_type, 'p_name': p_name, 'p_serial': p_serial, 'p_value': p_value}, function(data){
                                if (data) {
	...
	$(".j-showExtValPanel").live("click", function(){
		...
		$panel.find(".j-addExtVal").click(function(){
			...
			if(val!="" && val!=undefined){
                ...
                $.post("/McpItemAjax/add_prop_value", {prop_id: prop_id, prop_value: val, serial: serial}, function(data){
                    if (data) {
