...
function pullUpAction (){
	...
	console.log(total+'------'+firstRow)
	if(total > num*firstRow){
		$.post('/FrontUserCenter/load_my_recom_person', {"firstRow":num*firstRow}, function(data, textStatus)
		{
			console.log(data);
			if (data != 'failure')
			{
				...
				for (i = 0; i < len; i++)
				{
					html+='<div class="person_list clearfix">'+
						...
						'<span class="time">'+data[i].reg_time+'</span>'+
						'</div>'+
						'</div>'+
						'</div>';
				}
